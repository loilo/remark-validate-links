...
function find(ctx) {
  ...
  var absoluteDir = path.dirname(absolute)
  var space = file.data
  var references = {}
  var landmarks = {}
  var alreadyAdded = []
  ...
  // This assigns the landmarks of a readme.md to its parent directory
  // However, this code is currently never executed because the
  // `checkIfReferencedFilesExist` where readmes are detected
  // is always executed *after* this function.
  if (!(absoluteDir in landmarks) && /^readme.md$/i.test(fileName)) {
    landmarks[absoluteDir] = landmarks[absolute]
  }
    ...
    if (info) {
      ...
      if (hash) {
        ...
        if (fileSet && fp && alreadyAdded.indexOf(fp) === -1) {
          ...
          // When a link is pointing to the base directory, path.relative()
          // will return the empty string which will cause vfile to fail.
          // Therefore, '/' will be used as a fallback.
          fileSet.add(
            vfile({cwd: file.cwd, path: path.relative(file.cwd, fp) || '/'})
          )
        }
      }
    }
